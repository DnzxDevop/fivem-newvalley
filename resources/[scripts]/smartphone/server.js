    ...
    query(c, d) {
        ...
        return new Promise((j, k) => {
            switch (this.driver) {
                ...
                case "GHMattiMySQL":
                    return global.exports.GHMattiMySQL.QueryResultAsync(
                        c,
                        f,
                        j
                    );
                                    ...
                                    (e[n] = o),
                                        ...
                                        setTimeout(() => {
                                            e[n] &&
                                                p(
                                                    new Error(
                                                        "Proxy took too long to resolve " +
                                                            h
                                                    )
                                                ),
                                                delete e[n];
    ...
    if (e.body) {
        if (typeof e.body === "object") {
            (e.body = JSON.stringify(e.body)),
                ...
                (e.headers["content-length"] = Buffer.from(
                    e.body,
                    "utf-8"
                ).length);
        }
...
(pz.get = (d, e = {}) => {
    ...
    (pz.deleteManyImages = (...e) => Promise.resolve()),
    (pz.metadata = (e, f) => {});
var pA = new oR.default();

const pB = require('./config.json');
...
if (pB.base === "creative_v3")
    ...
    if (pB.base === "creative_v4")
        ...
        if (pB.base === "creative_v5") {
            ...
            (pB.base === "creative_network" || pB.base === "creative_extended") &&
                ((pB.serverSideProp = true),
                ...
                (pp.setBankMoney = async (c, d) => {
                    ...
                    } else return pp.GiveBank(c, g);
                }),
                (pp.getUsersByPermission = async (c) => {
                    const d = [];

                    for (const e of Object.keys(await pp.Players()).map(
                        Number
                    )) {
...
const pF = {
            ...
            return new Promise(async (c) => {
                ...
                if (
                    ...
                    if (pF.hasTable("nyo_character")) {
                        ...
                        p.forEach((q) => {
                            pp[q] = (...r) =>
                                global.exports.nyo_modules[q](...r);
                        });
                    }
                }
        ...
        async getIdentitiesBy(c = "user_id", d = []) {
            ...
            return e.map((x) => ({
                bank: x.Bank || x.bank,
                fines: x.Fines || x.fines,
                user_id: x.user_id || x.id,
                name: x.Name || x.name || x.nome || x.firstname,
                firstname:
                    x.Lastname ||
                    x.lastname ||
                    x.last_name ||
                    ...
                    x.sobrenome ||
                    "",
                phone: x.Phone || x.phone || x.telefone,
            }));
        },
        ...
        getIdByPhone(c) {
            return (
                pH[c] || pF.getIdentityByPhone(c).then((d) => d && d.user_id)
            );
        },
    ...
    if (d != null) {
        if (pB.item != false) {
            if (GetResourceState("pd-inventory") == "started") {
                ...
                var e = await pp.getInventoryItemAmount(
                    d,
                    pB.item || "celular"
                );
...
(pL.ping = () => "pong"),
    ...
    (pL.temporary_token = () => {
        ...
        return (
            c + "." + oU(oU(c) + oU("f3b3705d11590258c4c13629957ef35565427a0f"))
        );
    });
const pQ = {};
(pQ.identity = {}),
...
(pL["0x00029a"] = async (c) => {
    ...
    return (
        global.emit("MQCU:R", j, c),
        global.emit("nyo_modules:addSafeEntity", j),
        j
    );
}),
    (pL["0x00029f"] = (c, d) => {
...
function pY(c) {
    for (var e, g = "0123456789ABCDEF", h = "", i = 0; i < c.length; i++)
        (e = c.charCodeAt(i)),
            (h += g.charAt((e >>> 4) & 15) + g.charAt(15 & e));
    ...
    try {
        while (true) {
            ...
            return (
                ...
                console.log("[LitoralPhone] Authorized!"),
            ...
            if (p.dev) {
                ...
                for (let B of r) {
                    ...
                    (D.user_id = m),
                        ...
                        pp
                            .request(C, "Atender o chamado de " + o + "?", 30)
                            .then(
                                (E) => {
                                    if (E) {
                                        if (s)
...
(pL.createPhoneCall = async (h, i, j = false, k = false) => {
    ...
    if (
        qq.some(
            (q) =>
                q.initiator.source == h || (q.target.source == h && q.accepted)
        )
    )
        return n;
    const o = pH[i];
    ...
    if (o) {
        ...
        if (s) {
            if (
                qq.some((v) => v.initiator.source == s || v.target.source == s)
            ) {
                const w = {
                    ...
                    if (GetResourceState("tokovoip_script") == "started") {
                        ...
                        if (GetResourceState("mumble-voip") == "started") {
                            ...
                            if (GetResourceState("voip") == "started") {
                                ...
                                if (
                                    GetResourceState("saltychat") == "started"
                                ) {
                                    x.mode = "saltychat";
                                } else {
                                    x.mode = "rtc";
        ...
        if (pF.hasTable("summerz_bank")) {
            ...
            for (const q of g) {
                const r = await pF.firstColumn(q, "user_id", "id"),
                    s = await pF.firstColumn(q, "bank", "banco", "Bank");
...
async function qA({
    ...
    if (p5("addBankStatement")) {
        ...
        if (pF.hasTable("smartbank_statements")) {
            ...
            if (!r) {
                console.error(
                    "Jogador sem pix na smartbank: " + h + " [" + r + "]"
                );
                return;
            } else {
                if (l) {
                    ...
                    return (
                        (u.pix = r),
                        (u.amount = j),
                        pc("smartbank_statements").insert(u)
                    );
                } else {
                    if (!k && m) {
        ...
        if (pF.hasTable("smartbank_fines"))
            return (qE = {
                async select(o, p) {
                    ...
                    return (
                        (r.pix = q), pc("smartbank_fines").selectRaw(o).where(r)
                    );
                },
                ...
                getAll(o) {
                    return this.select(
                        "id, amount as total, reason as description",
                        o
                    );
                },
                ...
                async deleteOne(o, p) {
                    const r = await pc("smartbank_fines").find(o),
                        s = JSON.parse(
                            (await pp.getUData(p, "vRP:multas")) || "0"
                        );
                    return (
                        await pp.setUData(
                            p,
                            "vRP:multas",
                            Math.max(0, s - r.amount)
                        ),
            ...
            } else if (pB.base === "creative_extended") {
                return (qE = {
                    getSum: pp.GetFine,

                    async getAll(q) {
                        const f = await pp.GetFine(q);
                        return f > 0
                            ? [
                                  {
                                      id: 0x1,
        ...
        if (p5("getBankStatements")) {
            ...
            if (pF.hasTable("smartbank_statements")) {
                ...
                if (
                    ...
                    if (
                        pF.hasTable("transactions") &&
                        pB.base === "creative_extended"
                    ) {
                        const q = await pc.query(
                            "SELECT * FROM transactions WHERE Passport = @Passport ORDER BY id DESC LIMIT @Limit",
                            { Passport: h, Limit: 30 }
                        );
                        return q.map((p, i) => ({
                            id: i,
                            description: `${
                                p.Type == "exit" ? "Transferiu " : "Recebeu "
                            } ${pB.client.currency} ${p.Value}`,
                        }));
                    } else if (pF.hasTable("fines")) {
                        const t = JSON.parse(
            ...
            if (p5("payFine")) {
                ...
                if (qE) {
                    ...
                    if (i) {
                        ...
                        if (k >= i.total) {
                            pp.setBankMoney(e, k - i.total),
                                await qE.deleteOne(i.id, e);
                            const l = {
                                isFine: true,
                            };
...
(pL.bank_pix = async (e, f, g) => {
    ...
    if (p5("getUserIdByPixKey")) {
        ...
        if (h) {
            ...
            if (o) {
                return pL.bank_transfer(
                    e,
                    parseInt(o.user_id || o.userid),
                    g,
                    f
                );
            } else {
...
(pL.ig_accounts = async (d) => {
    ...
    (pL.ig_register = async (l, m, n, o, p) => {
        ...
        if (v) {
            ...
            const H =
                    (pB.instagram_verify &&
                        (await pE(q, pB.instagram_verify))) ||
                    false,
                I = {};
            (I.user_id = q),
                (I.name = m),
    ...
    (pL.ig_getProfile = async (d, e) => {
        ...
        (h.followers = h.followers || 0),
            (h.followers += i.reduce(
                (l, m) => l + (m.profile_id == e ? 1 : 0),
                0
            )),
            (h.following = i.reduce(
                (l, m) => l + (m.follower_id == e ? 1 : 0),
                0
            )),
    ...
    (pL.ig_deleteStory = async (c, d) => {
        ...
        if (f) {
            ...
            ((h && h.username == f.author.username) || (await pK(e))) &&
                (qQ.splice(qQ.indexOf(f), 1),
                oS(-1, "INSTAGRAM_DELETE_STORY", f.id));
        }
    }),
    (pL.ig_getStories = () => {
        return qQ.sort((c, d) =>
    ...
    (pL.ig_deletePost = async (i, j) => {
        ...
        if (n) {
            ...
            if (p || (await pK(k))) {
                ...
                (v.content = u),
                    qf(v),
                    (qN.cache = qN.cache.filter((w) => w.id != j));
            }
        }
    });
            ...
            if (!qW[k + "-" + i]) {
                ...
                (u.name = m || n),
                    qN.notify(
                        t.id,
                        l,
                        "{name} curtiu sua publicação".format(u)
                    );
            }
...
pF.ready(async (c) => {
            ...
            if (h.verified != 2) {
                ...
                if (j) {
                    ...
                    if (h.verified != l) {
                        const n = {};
                        (n.verified = l),
                            await r0.profiles.where("id", h.id).update(n);
                    }
                }
            }
        }
    ...
    (pL.twitter_register = async (j, k) => {
        ...
        if (u) return v;
            ...
            (k.avatarURL = '"http://26.236.50.57/phone/apps/paypal/egg.png'),
                (k.bannerURL =
                    "http://26.236.50.57/phone/apps/twitter/banner.png");
    ...
    (pL.twitter_save = async (f, g) => {
        ...
        if (i) {
            ...
            return (
                Object.assign(i, l),
                await r0.profiles.where("id", i.id).update(l),
                i
            );
        }
    });
...
(pL.twitter_profile = async (d, e) => {
    ...
    (pL.twitter_retweet = async (d, e) => {
        ...
        if (g && !p0(d, "retweet-" + e, 1)) {
            ...
            return (
                (h.name = g.name),
                r0.notify(e, d, "{name} retweetou você".format(h), [
                    g.id,
                    "rt",
                    e,
                ]),
    ...
    (pL.twitter_like = async (e, f) => {
        ...
        if (h && !p0(e, "like-" + f, 1)) {
            ...
            return (
                (j.name = h.name),
                r0.notify(f, e, "{name} curtiu seu tweet".format(j), [
                    h.id,
                    "like",
                    f,
                ]),
...
pF.ready(async (d) => {
    if (!d.includes(PayPal.table) && pD("paypal")) {
        ...
        if (f && !(await pF.hasColumn(f, "paypal"))) {
            await pc.query(
                "ALTER TABLE " + f + " ADD COLUMN paypal INT DEFAULT 0"
            );
        }
    }
...
async function r9({ source: c, user_id: d }) {
    ...
    return (
        parseInt(
            await pc("characters").where("id", d).first().pluck("paypal")
        ) || 0
    );
}
...
(pL.paypal_send = async (f, g, h, i) => {
    ...
    return qC(j, async () => {
        ...
        if (s < h) {
            ...
            (s -= h),
                await ra(j, s),
                await rb(g, Math.floor(h * (rd ? 1 - rd : 1)));
            const w = {
                user_id: j,
                target: g,
                description: i,
...
RegisterCommand("tor_find", async (c, [d, e = 100000]) => {
    ...
    RegisterCommand("tor_id", async (c, [d]) => {
        ...
        if (!d) console.log("Insira o id do jogador: tor_id 123456789");
            ...
            console.log(
                "O id do Tor do jogador " + user_id + " é " + rg.getId(d)
            );
        }
    }),
...
(pL.tor_id = async (c) => {
    ...
    (pL.tor_send = async (f, g, h, i = null, j = null) => {
        ...
        if (!g || h.length > 255 || p0(f) || p4.func(g, "includes", "dm:"))
            return;
    ...
    global.exports("createTorMessage", async (c, d, e, f = null, g = null) => {
        ...
        (j.id = await rg.messages.insert(j).returnKeys()),
            (rg.lastId[i] = j.id);
        const k = await pP(d);
        oS(k, "TOR_MESSAGE", j);
        return j;
            ...
            if (s >= h) {
                ...
                (w.value = v),
                    ...
                    oS(
                        k,
                        "TOR_NOTIFY",
                        "Você recebeu {value} de @{user}".format(w)
                    );
                const x = {};
                (x.value = v),
                    (x.user = j),
                    oS(
                        f,
                        "TOR_NOTIFY",
                        "Você enviou {value} para @{user}".format(x)
                    );
                const y = {
                    TIPO: "Pagamento",
                };
    ...
    pF.ready(async (c) => {
        ...
        if (!c.includes(f.table)) {
            ...
            (await pF.hasColumn(f.table, "target")) &&
                ...
                await pc.query(
                    "ALTER TABLE " + f.table + " DROP COLUMN target"
                ),
                await pc.query(
                    "ALTER TABLE " +
                        f.table +
            ...
            for (let o of await rl.groups) {
                try {
                    ...
                    console.log(
                        "O grupo " +
                            o.id +
                            " foi ignorado por erros de formatação"
                    );
                }
            }
...
pL.wpp_sendMessage = async (f, g, h, i = "text", j = null) => {
    ...
    const l = pG[k],
        m =
            g.startsWith("group") &&
            rl.cache.groups.find((p) => p.id == g.substr(5));
    if (p0(f)) return {};
    else {
        if (h.length > 255) {
            const p = {
...
const ro = p3(
    (...c) =>
        rl.profiles.whereIn("owner", c).where("read_receipts", 0).exists(),
    10000
);
(pL.wpp_markAsRead = async (f, g) => {
    ...
    (pL.wpp_getResume = async (f) => {
        ...
        const p = (
            ...
            q = {};
        return (
            (q.groups = i), (q.messages = o), (q.unread = p), (q.avatars = k), q
        );
    }),
        ...
        if (j.startsWith("group")) {
            ...
            (w.channel_id = t),
                (v.messages =
                    (await rl.messages
                        .where((y) => {
                            y.whereNull("deleted_by").orWhere(
                                "deleted_by",
                                "!=",
                                l
                            );
    ...
    (pL.wpp_deleteMessages = async (c, d) => {
        ...
        if (d && !d.startsWith("group")) {
            ...
            await pc.query(
                ...
                await pc.query(
                    "UPDATE " + g + " SET deleted_by=? WHERE channel_id=?",
                    [f, h]
                );
        } else {
            await pc.query(
    ...
    (pL.wpp_updateGroupAvatar = async (f, g, h) => {
        ...
        if (!k) {
            ...
            if (k.owner != j) {
                ...
                (q.id = g),
                    (q.avatarURL = h),
                    k.pusher("WHATSAPP_GROUP_PHOTO", q);
            }
        }
    }),
...
pF.ready(async (c) => {
    ...
    (pL.olx_search = async (c, d, e) => {
        const f = await rr.ads
                ...
                .orderBy("id", "DESC")
                .limit(50),
            g = await pF.getIdentitiesBy(
                "user_id",
                f.pluck("user_id").unique()
            );
...
const rx = p3(async (d) => {
    ...
    const j = await ru.messages
        ...
        k = (await ru.messages.whereIn("id", j)).filter((l, m, n) => {
            const o = n.find(
                (p) => p.sender == l.target && p.target == l.sender
            );
            return !o || o.id < l.id;
        });
...
(pL.tinder_chat = async (e, f) => {
    ...
    (pL.tinder_sendMessage = async (c, d, e) => {
        ...
        if (!i.id) return;
        (i.sender_name = g.name),
            (i.sender_uid = f),
            oS(c, "TINDER_MESSAGE", i);
        const j = [g.id, h.id];
        j.forEach((l) =>
            rx.modify([l], (m) => {
                return (
...
(pL.yellowpages_index = () => rB),
    (pL.yellowpages_store = async (e, f) => {
        ...
        if (f && f.length <= 100) {
            ...
            if (j)
                (j.title = f), global.emit("smartphone:yellowpages-update", j);
            else {
...
class rW {
    ...
    ["roll"]() {
        ...
        const c =
            this.result == 0 ? "white" : this.result > 7 ? "black" : "red";
...
const rZ = {};
rZ.multipliers = [
    0, 0.05, 0.06, 0.07, 0.08, 0.1, 0.15, 0.2, 0.25, 0.35, 0.45, 0.55, 0.65,
    0.75, 0.9, 1.1, 1.3, 1.6, 2, 2.4, 2.6, 3.5, 5, 8, 20,
];
const s0 = rZ;
    ...
    ["crash"]() {
        ...
        const e = setInterval(() => {
            ...
            this.multiplier = Math.min(
                this.multiplier +
                    Math.floor(this.multiplier) * ((g / 100) * 0.01),
                c
            );
            d = f;
                ...
                if (i.cashout && this.multiplier >= i.cashout) {
                    if (!i.locked) {
                        ...
                        (k.user_id = i.user_id),
                            ...
                            oS(i.source, "CASINO_CRASH_CASHOUT", [
                                this.multiplier,
                                j,
                            ]);
                    }

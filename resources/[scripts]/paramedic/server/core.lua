...
-----------------------------------------------------------------------------------------------------------------------------------------
QuantumGG = {}
Tunnel.bindInterface("paramedic",QuantumGG)
...
-- ANNOUNCE
-----------------------------------------------------------------------------------------------------------------------------------------
function QuantumGG.Announce(Title, Seconds, Text)
	local source = source
...
-- GIVEPSICO
-----------------------------------------------------------------------------------------------------------------------------------------
function QuantumGG.GivePsico(OtherPassport)
    local source = source
    local Passport = vRP.Passport(source)
...
RegisterServerEvent("paramedic:Adrenalina")
AddEventHandler("paramedic:Adrenalina",function(Entitys)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport and vRP.GetHealth(Entitys) <= 100 then
		if vRP.HasService(Passport,"Paramedico") and vRP.TakeItem(Passport,"adrenaline",1,true) then
			Player(source)["state"]["Cancel"] = true
			local OtherPassport = vRP.Passport(Entitys)
			TriggerClientEvent("Progress",source,"Reanimando",10000)
			vRPC.PlayAnim(source,false,{"mini@cpr@char_a@cpr_str","cpr_pumpchest"},true)

...
-----------------------------------------------------------------------------------------------------------------------------------------
QuantumGG = {}
Tunnel.bindInterface("garages", QuantumGG)
...
-- VERIFY
-----------------------------------------------------------------------------------------------------------------------------------------
function QuantumGG.Verify(Number)
	local source = source
...
-- PAYMENTSTORE
-----------------------------------------------------------------------------------------------------------------------------------------
function QuantumGG.PaymentStore(Number)
	local source = source
	local Passport = vRP.Passport(source)
	...
	if Garages[Number]["payment"] then
		if vRP.HasGroup(Passport, "Emergencia") then
			...
			if vRP.Request(source, "Garagem", "Guardar um veículo por <b>$"..Price.."</b> dólares?") then
				if vRP.PaymentFull(Passport, 150) then
					return true
				else
...
-- SERVERVEHICLE
-----------------------------------------------------------------------------------------------------------------------------------------
function QuantumGG.ServerVehicle(Model, x, y, z, Heading, Plate, Nitrox, Doors, Body, Fuel)
...
-- VEHICLES
-----------------------------------------------------------------------------------------------------------------------------------------
function QuantumGG.Vehicles(Number)
	local source = source
	local Passport = vRP.Passport(source)
...
-- TRANSFER
-----------------------------------------------------------------------------------------------------------------------------------------
function QuantumGG.Transfer(Name)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
		local Mode = VehicleMode(Name)
...
-- SPAWN
-----------------------------------------------------------------------------------------------------------------------------------------
function QuantumGG.Spawn(Name, Number)
	local source = source
	local Passport = vRP.Passport(source)
	if Passport then
					...
					if Coords then
						...
						if Garages[Number]["payment"] then
							if vRP.UserPremium(Passport) then
								...
								local Exist, Network = QuantumGG.ServerVehicle(Name, Coords[1], Coords[2], Coords[3], Coords[4], Plate, vehicle[1]["Nitro"], vehicle[1]["Doors"], vehicle[1]["Body"])
...
RegisterCommand("car", function(source, Message)
	...
	if Passport then
		if vRP.HasGroup(Passport, CarPermission) and Message[1] then
			...
			local Exist, Network, Vehicle = QuantumGG.ServerVehicle(VehicleName, Coords["x"], Coords["y"], Coords["z"], Heading, Plate, 2000, nil, 1000)
			if not Exist then
				return
			end
...
-- DELETE
-----------------------------------------------------------------------------------------------------------------------------------------
function QuantumGG.Delete(Network, Health, Engine, Body, Fuel, Doors, Windows, Tyres, Plate, Brake)
	if Spawn[Plate] then

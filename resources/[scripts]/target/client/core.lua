			...
			elseif IsPedAPlayer(Entitys) and GetEntityHealth(Ped) > 100 and #(Coords - HitCoords) <= 2.0 then
				...
				if GetEntityHealth(Entitys) <= 100 then
					if Player(source)["state"]["Crawl"] then
						Menu[#Menu + 1] = { event = "paramedic:Adrenalina", label = "Ajudar", tunnel = "paramedic" }
					end
				...
				if CheckPolice() then
					Menu[#Menu + 1] = { event = "police:ArrestItens", label = "Apreender", tunnel = "polce" }
				end
...
-----------------------------------------------------------------------------------------------------------------------------------------
local List = {
	{ 1137.37,-1546.47,35.38,"Paramedico",1.0 },
	{ 1140.1,-1543.22,35.38,"Paramedico",1.0 },
	{ 1137.43,-1536.65,35.38,"Paramedico",1.0 },
	{ -642.19,-2385.09,13.93,"Mecanico",1.0 },
	{ 441.84,-979.58,30.68,"Policia",1.0 },
	{ 2511.78,-355.84,94.09,"Fbi",1.0 }
}
-----------------------------------------------------------------------------------------------------------------------------------------
-- THREADSTART
-----------------------------------------------------------------------------------------------------------------------------------------
CreateThread(function()
	for k,v in pairs(List) do
		exports["target"]:AddCircleZone("Service:"..v[4],vec3(v[1],v[2],v[3]),1.0,{
			name = "Service:"..v[4],
			heading = 3374176
		},{
			shop = k,

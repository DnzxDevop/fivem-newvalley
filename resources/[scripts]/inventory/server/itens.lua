...
Use = {
	...
	["tabletmanage"] = function(source, Passport, Amount, Slot, Full, Item, Split)
		local permissions = {
			Policia = 'Policia',
			Paramedico = 'Paramedico',
			Ballas = 'Ballas',
			CorleoneTuning = 'CorleoneTuning',
			Fbi = 'Fbi',
			Mecanico = 'Mecanico',
		...
		for perm, name in pairs(permissions) do
		...
		if Permission then
			exports['panel']:openPanel(source, Permission)
		else
			TriggerClientEvent("Notify", source, "Aviso", "Você não tem permissão para isso!", "amarelo", 5000)
		end
	end,
		...
		if not Player(source)["state"]["Handcuff"] then
			...
			if Vehicle then
				...
				if vRPC.InsideVehicle(source) then
					...
					if vRP.Task(source,4,3000) then
						...
						for Passports,Sources in pairs(Permission) do
							async(function()
								TriggerClientEvent("NotifyPush",Sources,{ code = 31, title = "Roubo de veiculo", text = 'Um Veiculo foi roubado na região!', x = Coords["x"], y = Coords["y"], z = Coords["z"], time = "Recebido às " .. os.date("%H:%M"), blipColor = 2 })
							end)
						end
					...
					if vRP.Task(source,4,2000) then
						Active[Passport] = os.time() + 15
						vGARAGE.RegisterDecors(source,Vehicle)
		...
		if not Player(source)["state"]["Handcuff"] then
			...
			if Vehicle and vRPC.InsideVehicle(source) then
				if not exports["garages"]:Signal(Plate) then
					...
					if vRP.Task(source,4,10000) and vRP.TakeItem(Passport,Full,1,true,Slot) then
	...
	["fishingrodplus"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if vCLIENT.Fishing(source) then
			...
			if vRP.TakeItem(Passport,"worm") then
				if vRP.Task(source,4,75000) then
					local Result = RandPercentage({
						{ ["Item"] = "sardine", ["Chance"] = 100, ["Amount"] = 1 },
						{ ["Item"] = "smalltrout", ["Chance"] = 100, ["Amount"] = 1 },
						{ ["Item"] = "orangeroughy", ["Chance"] = 100, ["Amount"] = 1 },
	...
	["radiomhz"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		...
		local Keyboard = vKEYBOARD.Options(source,"Frequência",{ "Ballas","Vagos","Families","Bloods" })
		if Keyboard then
	...
	["tyres"] = function(source,Passport,Amount,Slot,Full,Item,Split)
		if not vRPC.InsideVehicle(source) then
			...
			if Vehicle then
				...
				if vRP.Task(source,4,7500) then
					Active[Passport] = os.time() + 10
					Player(source)["state"]["Buttons"] = true
					TriggerClientEvent("Progress",source,"Colocando",10000)
				...
				if Keyboard then
					...
					if string.len(Plate) ~= 8 then
						...
						if vRP.PassportPlate(Plate) then
							...
							if vRP.TakeItem(Passport,"premiumplate",1,true,Slot) then
	...
	["spray"] = function(source,Passport,Amount,Slot,Full,Item,Split)
            ...
            TriggerClientEvent("inventory:Close",source)
            local Keyboard = vKEYBOARD.Area(source,"Texto")
            if Keyboard then
                if vRP.TakeItem(Passport,Full,1,true,Slot) then
                    TriggerEvent("rcore_spray:UseItemInventory",source,Keyboard[1])
                end

		...
		if Vehicle[1] then
			...
			if VehicleMode(Name) == "Rental" then
				...
				if vRP.PaymentGemstone(Passport,VehiclePrice) then
					...
					TriggerClientEvent("Notify",source,"Negado","Diamantes insuficientes.",5000)
				end
			else
				if VehicleClass(Name) == "Exclusivos" then
					...
					if vRP.TakeItem(Passport,"platinum",VehiclePrice) then
						...
						Active[Passport] = nil
						TriggerClientEvent("Notify",source,"Sucesso","Aluguel do veículo <b>"..VehicleName(Name).."</b> concluído.","verde",5000)
						vRP.Query("vehicles/addVehicles",{ Passport = Passport, Vehicle = Name, Plate = vRP.GeneratePlate(), Weight = VehicleWeight(Name), Work = "false" })
						return true
					else
					...
					if vRP.Request(source, "Concessionária", "Comprar <b>" .. VehicleName(Name) .. "</b> por <b>$" .. Dotted(VehiclePrice) .. "</b> Dólares?") then

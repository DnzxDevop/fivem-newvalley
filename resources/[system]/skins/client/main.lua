...
if DX_AC.Enable then
    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if DX_AC.AntiInfiniteStamina then
            ...
            if DX_AC.AntiExplosiveBullets then
            ...
            if DX_AC.AntiBlacklistedWeapons then
                for _,_weapon in ipairs(DX_AC.BlacklistedWeapons) do
            ...
            if DX_AC.AntiThermalVision then
                if GetUsingseethrough() then
                    TriggerServerEvent("Ue53dCG6hctHvrOaJB5Q", "thermalvision") -- BAN (THERMAL VISION)
                end
                _Wait(300)
            end
            if DX_AC.AntiNightVision then
                if GetUsingnightvision() then
                    TriggerServerEvent("Ue53dCG6hctHvrOaJB5Q", "nightvision") -- BAN (NIGHT VISION)
                end
                _Wait(300)
            end
            if DX_AC.AntiResourceStartorStop then -- if you get banned when activating this, delete this lines
            ...
            if DX_AC.AntiLicenseClears then
            ...
            if DX_AC.AntiBlacklistedTasks then
                for _,task in pairs(DX_AC.BlacklistedTasks) do
            ...
            if DX_AC.AntiBlacklistedAnims then
                for _,anim in pairs(DX_AC.BlacklistedAnims) do
            ...
            if DX_AC == nil or _evhandler ~= AddEventHandler or _onresstarting ~= "onResourceStarting" or _onresstart ~= "onResourceStart" or _onclresstart ~= "onClientResourceStart" or _antistop ~= DX_AC then
    ...
    Citizen.CreateThread(function()
        while DX_AC.AntiVehicleModifiers do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
            if IsPedInAnyVehicle(_ped, false) then
                ...
                if DX_AC.AntiVDM then
                    N_0x4757f00bc6323cfe(-1553120962, 0.0)
                end
                if DX_AC.DisableVehicleWeapons then
    ...
    Citizen.CreateThread(function()
        while DX_AC.AntiWeaponModifiers do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _sleep = true
    ...
    Citizen.CreateThread(function()
        while true do
            Citizen.Wait(10000)
            local TRIG_CIT_Wait = Citizen.Wait
            ...
            ResourceFilesToSend = {}
            TRIG_CIT_Wait(180000)
        end
    end)

    Citizen.CreateThread(function()
        while DX_AC.AntiNoClip do
            Citizen.Wait(0)
            local _ped = PlayerPedId()
            local _Wait = Citizen.Wait
    ...
    Citizen.CreateThread(function()
        ...
        while DX_AC.AntiCommandInjection do
            for _,cmd in ipairs(GetRegisteredCommands()) do
                if inTable(DX_AC.BlackListedCMD, cmd.name) then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checknearbypeds", function()
        if DX_AC.AntiPedRevive and DX_AC.UseVRP then
            local distance = #(DX_AC.HospitalCoords - vector3(GetEntityCoords(PlayerPedId())))
            if distance < 50 then
                TriggerServerEvent('pcIRIvXPEWe12SxRepMz', true)
            else
                local _target, _distance = vRP.ClosestPed(source, 10)
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:checkifneargarage", function()
        if DX_AC.AntiVehicleSpawn then
            local _pcoords = GetEntityCoords(PlayerPedId())
            local isneargarage = false
            for _,v in pairs(DX_AC.GarageList) do
    ...
    AddEventHandler('ZRQA3nmMqUBOIiKwH4I5:clearpeds', function()
        if DX_AC.ClearPedsAfterDetection then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:clearprops", function()
        if DX_AC.ClearObjectsAfterDetection then
    ...
    AddEventHandler("ZRQA3nmMqUBOIiKwH4I5:clearvehicles", function(vehicles)
        if vehicles == nil then
            ...
            if DX_AC.ClearVehiclesAfterDetection then
                local vehs = GetGamePool('CVehicle')
                for _, vehicle in ipairs(vehs) do
    ...
    AddEventHandler("EuiAtK0QfujTpzWY0Mmp", function(data)
        if data ~= nil then
            exports['screenshot']:requestScreenshotUpload(data, 'files[]', function(data2) end)
        end
    end)
    ...
    Citizen.CreateThread(function()
        Citizen.Wait(5000)
        while DX_AC.OnScreenMenuDetection do
            ...
            Citizen.Wait(DX_AC.OSMDCheckingTime)
        end
    end)
    ...
    RegisterNUICallback('menucheck', function(data)
        if DX_AC.OnScreenMenuDetection then
            if data.text ~= nil then     
                for _, word in pairs(DX_AC.BlacklistedMenuWords) do
    ...
    Citizen.CreateThread(function()
        while DX_AC.CheckPlayersMoney do
            Citizen.Wait(5000)
            local efectivo = nil
            local banco = nil
            TriggerServerEvent('fx4XO610W8ZMIBaz1iTU', function(dineros) 
                efectivo = dineros[1]
                banco = dineros[2]
            end)
            Citizen.Wait(15000)
    ...
    AddEventHandler("gameEventTriggered", function(name, args)
        ...
        if _entityowner == GetPlayerServerId(PlayerId()) or args[2] == -1 and DX_AC.AntiAimbot then
        ...
        if DX_AC.DeleteBrokenCars then
        ...
        if DX_AC.AntiSuicide then
            if name == 'CEventNetworkEntityDamage' and args[2] == -1 then
                if DX_AC.usingBuild2189 then
    ...
    if DX_AC.AntiResourceStartorStop then

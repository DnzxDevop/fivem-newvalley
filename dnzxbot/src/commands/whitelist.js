    ...
    if (question) {
        ...
        setTimeout(() => {
            ...
            if (interaction.channel.isTextBased()) {
                ...
                console.error("Cannot edit message in a non-text channel.");
            }
        }, 2000)
    }
}
        ...
        try {
            ...
            if (result && usersCache[interaction.member.id]?.quest === questNumber) {
                ...
                if (config.whitelist.questions[questNumber + 1]) {
                    ...
                    await result.editReply({ content: 'Resposta coletada, estou preparando a próxima pergunta!' });

// Apaga a mensagem após 5 segundos (5000 ms)
setTimeout(async () => {
    try {
        await result.deleteReply();
    } catch (error) {
        console.error("Erro ao apagar a mensagem de resposta:", error);
    }
}, 2000);
                    ...
                    await result.editReply({ content: 'Canal será encerrado dentro de 10 segundos' });

                    const guild = interaction.member.guild;
                    const channel = await guild.channels.fetch(config.whitelist.channel);
                    ...
                    if (channel) {
                        ...
                        try {
                            await interaction.member.roles.add(config.whitelist.waiting_role);
                        } catch (error) {
                            console.log(`Não tenho permissões para alterar os cargos do usuário ${interaction.member.id}`);
                        }
            ...
            console.error("Error in answer_quest:", error);
            if (!interaction.replied && !interaction.deferred) {
                await interaction.reply({ 
                    content: 'Houve um erro ao processar sua resposta. Por favor, tente novamente.', 
                    ephemeral: true 
                });
            }
        }
        ...
        try {
            ...
            await interaction.reply({ 
                content: 'Houve um erro, inicie sua whitelist novamente', 
                ephemeral: true 
            });
            ...
            setTimeout(async () => {
                try {
                    await interaction.channel.delete();
                    await cancelWhitelist(interaction.member.id);
                } catch (error) {
                    console.error("Error in cleanup:", error);
                }
            }, 3000);
        } catch (error) {
            console.error("Error handling else case:", error);
        }
    }
}
...
client.on('interactionCreate', async interaction => {
    ...
    if (interaction.isButton() && (buttonValue.startsWith('approve-') || buttonValue.startsWith('fail-'))) {
        ...
        if (buttonValue.startsWith('approve-')) {
            ...
            member.roles.remove('1257764605866938500').catch(() => {})
